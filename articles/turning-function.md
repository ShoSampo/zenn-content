...
1. **非類似度行列の作成**
   - 全てのポリゴンペアの組み合わせについてTurning functionのL2ノルム（非類似度）を計算
   - N×Mの非類似度行列を生成（Nは群A、Mは群Bのポリゴン数）
...
| 非類似度（L2ノルム） | 平行四辺形 | 五角形 | 矢印 | 稲妻 | 星型 | K字 |
|-|-|-|-|-|-|-|
...
# 次回予告
次回は「Turning Functionとアフィン変換を用いた地図ポリゴンのフィッティング」として、より実践的な地図ポリゴンのフィッティングについて解説します。具体的には以下のトピックを扱います：

- Shi-Tomasiコーナー検出を用いた特徴点抽出
- 特徴点周辺のTurning Function解析によるマッチング
- アフィン変換を用いた地図ポリゴンの位置合わせ
